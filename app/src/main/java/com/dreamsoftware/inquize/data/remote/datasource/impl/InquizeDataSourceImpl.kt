...
internal class InquizeDataSourceImpl(
    ...
    override suspend fun addMessage(data: AddInquizeMessageDTO): Unit = withContext(dispatcher) {
        try {
            val messages = addInquizeMessageMapper.mapInToOut(data)
            val documentRef = userQuestionsCollection
                .document(data.userId)
                .collection(SUB_COLLECTION_NAME)
                .document(data.uid)
            ...
            throw AddInquizeMessageRemoteDataException("Failed to save user question", e)
        }
    }
